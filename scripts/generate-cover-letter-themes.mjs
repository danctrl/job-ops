...
const themesDir = join(
  repoRoot,
  "orchestrator/src/server/services/resume-renderer/cover-letter-themes",
);
const outputPath = join(
  repoRoot,
  "shared/src/generated/cover-letter-themes.ts",
);
...
async function readThemeManifests() {
  ...
  const themeDirNames = entries
    ...
    .sort();
  assert(themeDirNames.length > 0, "No cover letter theme directories found");
  ...
  for (const themeDirName of themeDirNames) {
    ...
    assert(
      !ids.has(manifest.id),
      `Duplicate cover letter theme id: ${manifest.id}`,
    );
    ids.add(manifest.id);
...
export const COVER_LETTER_THEME_VALUES = ${formatArray(values)} as const;
...
if (checkOnly) {
  ...
  if (current !== generated) {
    throw new Error(
      `${relative(repoRoot, outputPath)} is stale. Run npm run cover-letter-theme:generate.`,
    );
  }
  console.log(`Validated ${manifests.length} cover letter theme manifests.`);
} else {
  await writeFile(outputPath, generated, "utf8");
  console.log(
    `Generated ${relative(repoRoot, outputPath)} from ${manifests.length} cover letter themes.`,
  );
}
